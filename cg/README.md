...
For dev setup you can simply copy the 3 example .env files:
...
These files have all the variables required for the dev environment to be properly set up.

### Deploy the contract:

Then after copying example `.env` files you'll need to deploy our first contract and fill in some additional unset env variables.

To deploy the contract you'll need to make a request to `/api/management/contract/deploy`.
A cURL command with a proper token for our .env.example and current contract looks like this:

```bash
curl -X POST \
  http://localhost:8082/api/management/contract/deploy \
  -H 'Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MSwiaWF0IjoxNTI0NDkxNDcxfQ.PL0n24IqoMo3BenFNK3eAcJ7ae8i2FQQgamuzujCjOk' \
...
It will seed 3 example processors with the following addresses:
